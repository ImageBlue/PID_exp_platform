...
//const char* mqttServer = "172.6.5.141";  //要连接到的服务器IP
...
void callback(char*topic, byte* payload, unsigned int length) 
{
  ...
  //目标流速设置
  if(mess[0]=='V' && mess[1]=='E' && mess[2]=='L' && mess[3]=='O')  //"VELO xxx"  data:5 6 7
  {
    if(!mess[6])
      target_velo=mess[5]-48;
    else if(!mess[7])
      target_velo=(mess[5]-48)*10+mess[6]-48;
    else if(!mess[8])
      target_velo=(mess[5]-48)*100+(mess[6]-48)*10+mess[7]-48;
    mtv = target_velo*1.0;
    pid1.set_target(mtv);
  }
